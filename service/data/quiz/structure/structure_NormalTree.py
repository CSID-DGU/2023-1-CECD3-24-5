...
def generate_not_bst_conditions(node):
    conditions=[]
    if node.left:
...
class NormalTree:
    def __init__(self):
        ...
        # bst 위반
        not_bst_conditions = Or(generate_not_bst_conditions(self.a))
        not_heap_conditions =Or(generate_not_minHeap_conditions(self.a))
        ...
        # 이진 탐색 트리 조건 확인
        s.push()
        s.add(And(not_bst_conditions,not_heap_conditions, unique_conditions))
        for _ in range(10):  # 10개의 서로 다른 모델 뽑아냄
            if s.check() == sat:
                m = s.model()
                normalTree_models.append(m)
        ...
        # 가능한 해 중 하나를 무작위로 선택
        selected_model = random.choice(normalTree_models)
